...
raw_data = pd.read_excel(r'taa_data.xlsx', sheetname = 'TRET', index_col = 'Date')
...
ret_data = ret_data['1989-12':]
t_bill.index = ret_data.index[:-1]
ret = pd.concat([ret_data['SnP500'],t_bill['t_bill']], axis= 1)
ret = ret['1990-1-01':]
ret = ret.drop(ret.index[-1])
ret['timing'] = pd.Series()
price['diff'] = price.iloc[:,0] - price.iloc[:,1]
price_modi = price['1990-1-01' : ]
...
for i in range(len(ret)):
    if ret.iloc[:,0][i] > 0:
        count = count +1
snp_positive_month = count /len(ret)
count1 = 0
for i in range(len(timing)):
    if timing.iloc[:,0][i]> 0:
        count1 = count1 +1
timing_positive_month = count1 /len(timing) 
...
timing_cum_ret =100* (timing[['timing']]+1).cumprod()
plt.plot(snp_cum_ret[:'2012'],'b', label= 'SnP500', alpha = 0.6)
plt.plot(timing_cum_ret[:'2012'], 'r' , label ='timing', alpha = 0.6)
...
description = ret_concat.describe(include='all')

...
def Assetselection(data,bondfraction,TOP):
    ...
    for i in range(len(Assetallocation)):
        if Assetallocation['KODEX 200'].iloc[i] + Assetallocation['TIGER 코스닥150'].iloc[i]<0.1:
            Assetallocation['KODEX 200'].iloc[i] = 0.1
        if Assetallocation['TIGER 미국S&P500선물(H)'].iloc[i] + Assetallocation['TIGER 유로스탁스50(합성 H)'].iloc[i]+Assetallocation['KINDEX 일본Nikkei225(H)'].iloc[i]+Assetallocation['TIGER 차이나CSI300'].iloc[i] < 0.1:
            Assetallocation['TIGER 미국S&P500선물(H)'].iloc[i]= 0.1/4
            Assetallocation['TIGER 유로스탁스50(합성 H)'].iloc[i]= 0.1/4
            Assetallocation['KINDEX 일본Nikkei225(H)'].iloc[i]= 0.1/4
            Assetallocation['TIGER 차이나CSI300'].iloc[i] = 0.1/4
        if Assetallocation['KOSEF 국고채10년'].iloc[i] + Assetallocation['KBSTAR 중기우량회사채'].iloc[i] + Assetallocation['TIGER 단기선진하이일드(합성 H)'].iloc[i] < 0.2:
            Assetallocation['KOSEF 국고채10년'].iloc[i] = 0.2
        if Assetallocation['KOSEF 국고채10년'].iloc[i] + Assetallocation['KBSTAR 중기우량회사채'].iloc[i] + Assetallocation['TIGER 단기선진하이일드(합성 H)'].iloc[i] > 0.6 :
            Assetallocation['KOSEF 국고채10년'].iloc[i] = Assetallocation['KOSEF 국고채10년'].iloc[i]- (Assetallocation['KOSEF 국고채10년'].iloc[i] + Assetallocation['KBSTAR 중기우량회사채'].iloc[i] + Assetallocation['TIGER 단기선진하이일드(합성 H)'].iloc[i]-0.6)
        if Assetallocation['KODEX 골드선물(H)'].iloc[i] + Assetallocation['TIGER 원유선물Enhanced(H)'].iloc[i] < 0.05:
            Assetallocation['KODEX 골드선물(H)'].iloc[i] = 0.05
        if Assetallocation.iloc[i].sum(0) > 1 :
            Assetallocation['KOSEF 국고채10년'].iloc[i] =  Assetallocation['KOSEF 국고채10년'].iloc[i] - (Assetallocation.iloc[i].sum(0)-1)
